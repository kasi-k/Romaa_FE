...
const UploadAbstract = ({ onclose, onSuccess, name, hasExistingData = false, existingCount = 0 }) => {
    ...
    try {
      ...
      if (files.length === 1) {
        ...
        const res = await axios.post(
          `${API}/detailedestimate/bulkinsertcustomheadnew?tender_id=${tender_id}&replace=${hasExistingData}`,
          formData,
          {
        ...
        if (res.data.status) {
          ...
          setTimeout(() => {
            if (onclose) onclose();
            toast.success(hasExistingData ? "Existing data replaced successfully" : "Files uploaded successfully");
          }, 1500);
        } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-black/30 animate-in fade-in duration-200">
      <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 border border-gray-100 dark:border-slate-800">
        ...
        >
          {/* Replace warning */}
          {hasExistingData && (
            <div className="mb-4 flex items-start gap-2.5 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/40 rounded-xl px-4 py-3">
              <span className="text-amber-500 mt-0.5 shrink-0">⚠</span>
              <p className="text-xs font-semibold text-amber-700 dark:text-amber-400">
                {existingCount} existing record{existingCount !== 1 ? "s" : ""} will be replaced with the new upload.
              </p>
            </div>
          )}
